    ...
    <div x-data="orderForm()" x-init="init()">
            ...
            <div x-show="patient" x-cloak>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                    ...
                    <div class="bg-blue-50 rounded-lg p-3">
                        ...
                        <p class="font-semibold text-gray-800 text-sm"
                            x-text="(patient?.age ?? '—') + ' yrs / ' + (patient?.gender ?? '—')"></p>
                    </div>
                    <div class="bg-blue-50 rounded-lg p-3">
                        <p class="text-xs text-blue-600 mb-0.5">BSA</p>
                        <p class="font-semibold text-gray-800 text-sm" x-text="bsa ? bsa + ' m²' : '—'"></p>
                        <p class="text-xs text-blue-500 mt-0.5">CrCl: <span x-text="crcl ? crcl + ' mL/min' : '—'"></span>
                        </p>
                    </div>
                </div>
                ...
                <div class="border border-blue-200 rounded-lg bg-blue-50/50 p-4">
                    ...
                    <div class="mt-2 flex gap-2">
                        ...
                        <span class="text-xs text-gray-400 self-center">Changes are saved to patient record and doses
                            recalculate immediately.</span>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-4">
            ...
            <div x-show="drugs.length" x-cloak class="mt-3 flex items-start gap-3 border-t border-gray-100 pt-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">Check this when the cycle is divided into multiple weekly/daily
                        doses. The system counts it as one cycle.</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-4" x-show="drugs.length" x-cloak>
            ...
            <p class="text-xs text-gray-500 mb-3">
                ...
                To reduce a specific drug's dose, change its <strong>Mod %</strong> in the drug table. Each drug can be
                modified independently. Leave at 100% for full protocol dose.
            </p>
            <div>
                            ...
                            <thead class="bg-gray-50 text-gray-400 uppercase">
                                <tr>
                                    ...
                                    <th class="px-3 py-2 text-left w-20"
                                        x-show="category !== 'pre_medication' && category !== 'post_medication' || true">
                                        Mod %</th>
                                ...
                                <template x-for="drug in drugsByCategory(category)" :key="drug.protocol_drug_id">
                                    <tr :class="!drug.is_included ? 'opacity-50 bg-gray-50' : ''">
                                        ...
                                        <td class="px-3 py-2 text-gray-500"
                                            x-text="drug.duration_days ? drug.duration_days + 'd' : '—'"></td>
                                        <td class="px-3 py-2 text-blue-600 font-mono"
                                            x-show="category === 'chemotherapy'">
                                            <span
                                                x-show="drug.dose_type !== 'fixed' && drug.dose_type !== 'carboplatin_calvert'"
                                                x-text="drug.dose_per_unit ? parseFloat(drug.dose_per_unit).toFixed(2) + ' ' + doseUnitLabel(drug.dose_type) : '—'"></span>
                                        ...
                                        <td class="px-3 py-2">
                                            <div class="flex flex-wrap gap-1">
                                                <span x-show="drug.cap_applied"
                                                    class="text-xs bg-yellow-100 text-yellow-700 px-1 py-0.5 rounded"><i
                                                        class="fa-solid fa-circle-minus"></i> Cap</span>
                                                <span x-show="drug.modification_pct != 100 && !drug.is_manually_overridden"
                                                    class="text-xs bg-orange-100 text-orange-700 px-1 py-0.5 rounded"><span
                                                        x-text="drug.modification_pct + '%'"></span></span>
                                                <span x-show="drug.is_manually_overridden"
                                                    class="text-xs bg-red-100 text-red-700 px-1 py-0.5 rounded"><i
    ...
    <script>
        function orderForm() {
            return {
                ...
                init() {
                    if (this.mrnInput) this.lookupMrn();
                },

...
<div x-data="orderForm()" x-init="init()">
    ...
    <div x-show="showCapModal" x-cloak class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-xl max-w-lg w-full p-6">
            ...
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-sm text-yellow-800 mb-5">
                <i class="fa-solid fa-circle-info mr-1"></i> By confirming, you acknowledge this override and accept clinical responsibility.
            </div>
            <div class="flex gap-3">
                ...
                <button type="button" @click="showCapModal = false" class="flex-1 border border-gray-200 text-gray-600 text-sm py-2 rounded-lg hover:bg-gray-50 transition">Cancel</button>
            </div>
        </div>
    </div>

    {{-- Protocol Modified Alert Banner --}}
    <div x-show="isProtocolModified" x-cloak
        ...
        <div>
            ...
            <p class="text-xs text-orange-700">One or more drugs have been adjusted from the standard protocol doses, frequencies, or durations.</p>
        </div>
    </div>
            ...
            <div class="border border-blue-200 rounded-lg bg-blue-50/40 p-4">
                ...
                <div class="grid grid-cols-3 gap-3">
                    <div>
                        ...
                        <input type="number" step="0.1" x-model="quickEdit.height_cm" class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Weight (kg)</label>
                        <input type="number" step="0.01" x-model="quickEdit.weight_kg" class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Serum Creatinine (µmol/L)</label>
                        <input type="number" step="0.01" x-model="quickEdit.serum_creatinine" class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                <button type="button" @click="saveQuickEdit()" class="mt-2 bg-blue-600 hover:bg-blue-700 text-white text-xs px-4 py-1.5 rounded transition">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-4">
        ...
        <div x-show="cycleInfo" x-cloak class="mt-3 flex items-center gap-3 flex-wrap">
            ...
            <span x-show="cycleInfo?.is_same_cycle" class="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">
        ...
        <div x-show="drugs.length" x-cloak class="mt-3 flex items-start gap-3 border-t border-gray-100 pt-3">
            <input type="checkbox" id="split_cycle" x-model="isSplitCycle" class="mt-0.5 rounded border-gray-300 text-blue-600">
            <div>
    ...
    {{-- STEP 3: Dose Modification Reason --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-4" x-show="drugs.length" x-cloak>
        ...
        <p class="text-xs text-gray-500 mb-3">
            ...
            Adjust each drug's <strong>Mod %</strong>, Frequency, Duration, or Final Dose independently. Any change will trigger the "Protocol Modified" alert.
        </p>
        <div>
    ...
    {{-- PRE-MEDICATIONS: Card style --}}
    <div x-show="drugsByCategory('pre_medication').length" x-cloak class="bg-white rounded-xl shadow-sm border border-gray-100 mb-4">
        <div class="px-5 py-3 border-b border-gray-100 flex items-center gap-2">
            ...
            <span class="ml-1 text-xs bg-green-100 text-green-700 rounded-full px-2 py-0.5" x-text="drugsByCategory('pre_medication').length"></span>
        </div>
        <div class="p-4 space-y-3">
            <template x-for="drug in drugsByCategory('pre_medication')" :key="drug.protocol_drug_id">
                <div class="border border-gray-200 rounded-xl p-4" :class="!drug.is_included ? 'opacity-50 bg-gray-50' : 'bg-white'">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-2">
                            <input type="checkbox" x-model="drug.is_included" class="rounded border-gray-300 text-blue-600">
                            <span class="font-semibold text-gray-800 text-sm" x-text="drug.drug_name + ' (' + drug.drug_unit + ')'"></span>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-3 text-xs">
                        ...
                        <div>
                            ...
                            <input type="text" x-model="drug.physician_frequency"
                                :placeholder="drug.frequency || 'e.g. Once daily'"
                                @input="checkModified()"
                                class="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                                :class="drug.physician_frequency && drug.physician_frequency !== drug.frequency ? 'border-orange-300 bg-orange-50' : ''">
                        </div>
                        <div>
                            <label class="block text-gray-500 mb-1">Route</label>
                            <span class="block px-2 py-1.5 text-sm text-gray-600 bg-gray-50 rounded-lg border border-gray-200" x-text="drug.route || '—'"></span>
                        </div>
                        <div>
                            <label class="block text-gray-500 mb-1">Duration (days)</label>
                            <input type="text" x-model="drug.physician_duration"
                                :placeholder="drug.duration_days || 'e.g. 5'"
                                @input="checkModified()"
                                class="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                                :class="drug.physician_duration && drug.physician_duration !== drug.duration_days ? 'border-orange-300 bg-orange-50' : ''">
                        </div>
                        <div>
    ...
    {{-- CHEMOTHERAPY: Table style --}}
    <div x-show="drugsByCategory('chemotherapy').length" x-cloak class="bg-white rounded-xl shadow-sm border border-gray-100 mb-4">
        <div class="px-5 py-3 border-b border-gray-100 flex items-center gap-2">
            <i class="fa-solid fa-circle text-xs text-red-500"></i>
            <h3 class="font-semibold text-gray-700 text-sm">Chemotherapy</h3>
            <span class="ml-1 text-xs bg-red-100 text-red-700 rounded-full px-2 py-0.5" x-text="drugsByCategory('chemotherapy').length"></span>
        </div>
                        ...
                        <tr :class="!drug.is_included ? 'opacity-50 bg-gray-50' : ''">
                                ...
                                <template x-if="drug.dose_type !== 'fixed'">
                                    <div class="flex items-center gap-0.5">
                                        <input type="number" x-model="drug.modification_pct"
                                            @input="applyDrugModification(drug); checkModified()"
                                            ...
                                            :class="drug.modification_pct!=100?'border-orange-400 bg-orange-50 text-orange-700 font-bold':'border-gray-300'">
                            ...
                            <td class="px-3 py-2">
                                <input type="text" x-model="drug.physician_frequency"
                                    ...
                                    :class="drug.physician_frequency && drug.physician_frequency!==drug.frequency?'border-orange-300 bg-orange-50':'border-gray-300'">
                            </td>
                            <td class="px-3 py-2">
                                <input type="text" x-model="drug.physician_duration"
                                    :placeholder="drug.duration_days || '—'"
                                    @input="checkModified()"
                                    class="w-16 border rounded px-1.5 py-0.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500"
                                    :class="drug.physician_duration && drug.physician_duration!==drug.duration_days?'border-orange-300 bg-orange-50':'border-gray-300'">
                            </td>
                            <td class="px-3 py-2">
                                <div class="flex flex-wrap gap-1">
                                    <span x-show="drug.cap_applied" class="text-xs bg-yellow-100 text-yellow-700 px-1 py-0.5 rounded"><i class="fa-solid fa-circle-minus"></i></span>
                                    <span x-show="drug.modification_pct!=100&&!drug.is_manually_overridden" class="text-xs bg-orange-100 text-orange-700 px-1 py-0.5 rounded" x-text="drug.modification_pct+'%'"></span>
                                    <span x-show="drug.is_manually_overridden" class="text-xs bg-red-100 text-red-700 px-1 py-0.5 rounded"><i class="fa-solid fa-pen"></i></span>
                                </div>
                            </td>
                        </tr>
    ...
    {{-- POST-MEDICATIONS: Card style --}}
    <div x-show="drugsByCategory('post_medication').length" x-cloak class="bg-white rounded-xl shadow-sm border border-gray-100 mb-4">
        <div class="px-5 py-3 border-b border-gray-100 flex items-center gap-2">
            <i class="fa-solid fa-circle text-xs text-blue-500"></i>
            <h3 class="font-semibold text-gray-700 text-sm">Post-Medications</h3>
            <span class="ml-1 text-xs bg-blue-100 text-blue-700 rounded-full px-2 py-0.5" x-text="drugsByCategory('post_medication').length"></span>
        </div>
        <div class="p-4 space-y-3">
...
function orderForm() {
    return {
        ...
        quickEdit: { height_cm: '', weight_kg: '', serum_creatinine: '' },
        quickSaved: false,
        diagnosisId: '',
        protocolId: '',
        protocols: [],
        ...
        async lookupMrn() {
            ...
            if (!res.ok) { this.patient = null; this.patientNotFound = true; return; }
        ...
        async loadProtocols() {
            this.protocolId = ''; this.drugs = []; this.cycleInfo = null; this.testsReminder = null;
        ...
        async loadDrugTable() {
            ...
            const res = await fetch('/api/orders/calculate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content },
                body: JSON.stringify({ patient_id: this.patient.id, protocol_id: this.protocolId }),
            });
            const data = await res.json();
            this.bsa = data.bsa; this.crcl = data.crcl; this.cycleInfo = data.cycle_info;
        ...
        applyDrugModification(drug) {
            ...
            if (drug.per_cycle_cap && newFinal > drug.per_cycle_cap) { newFinal = drug.per_cycle_cap; drug.cap_applied = true; }
        ...
        doseUnitLabel(dt) {
            return { bsa_based: 'mg/m²', weight_based: 'mg/kg', crcl_based: 'mg/mL/min' }[dt] || '';
        },
        ...
        async submitOrder() {
            ...
            if (data.requires_acknowledgment) { this.capWarnings = data.lifetime_warnings; this.showCapModal = true; this._pendingPayload = payload; }
            else if (data.redirect) { window.location.href = data.redirect; }
        },

        async acknowledgeAndSubmit() {
            this.showCapModal = false; this.submitting = true;

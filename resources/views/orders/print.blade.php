...
<body class="bg-white text-gray-900 p-6">
    ...
    <div class="no-print flex gap-3 mb-6">
        ...
        <a href="{{ route('orders.show', $order) }}" class="border border-gray-200 text-gray-600 text-sm px-5 py-2 rounded-lg hover:bg-gray-50">
    ...
    <div class="border border-gray-300 rounded mb-4">
        ...
        <div class="grid grid-cols-4 gap-0 text-xs">
            ...
            <div class="border-r border-gray-200 px-3 py-2">
                ...
                <p class="font-bold">{{ $order->patient->date_of_birth->format('d M Y') }} / {{ ucfirst($order->patient->gender) }}</p>
            </div>
            <div class="px-3 py-2">
    ...
    @if($catDrugs->count() > 0)
    <div class="border border-gray-300 rounded mb-4">
        <div class="px-3 py-1 border-b border-gray-300 {{ $cat === 'chemotherapy' ? 'bg-red-50' : 'bg-gray-100' }}">
            <p class="text-xs font-bold uppercase tracking-wide {{ $cat === 'chemotherapy' ? 'text-red-700' : 'text-gray-600' }}">
                @if($cat === 'chemotherapy') <i class="fa-solid fa-triangle-exclamation mr-1"></i> @endif
                {{ $catLabel }}
            </p>
        </div>
        <table class="w-full text-xs">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-3 py-1.5 text-right font-semibold text-gray-600 {{ $cat === 'chemotherapy' ? 'text-red-700 bg-red-50' : '' }}">Final Dose</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="{{ $od->is_manually_overridden ? 'bg-orange-50' : '' }}">
                    ...
                    <td class="px-3 py-2 text-right font-mono text-gray-500">{{ number_format($od->calculated_dose, 2) }}</td>
                    <td class="px-3 py-2 text-right font-mono font-bold {{ $od->is_manually_overridden ? 'text-orange-700' : '' }} {{ $cat === 'chemotherapy' ? 'text-red-800' : '' }}">
                        {{ number_format($od->final_dose, 2) }}
                    </td>
                    <td class="px-3 py-2">{{ $od->drug->unit }}</td>
                    <td class="px-3 py-2">
                        @if($od->cap_applied) <span class="bg-yellow-100 text-yellow-700 px-1 py-0.5 rounded text-xs mr-1">Cap Applied</span> @endif
                        @if($od->is_manually_overridden) <span class="bg-orange-100 text-orange-700 px-1 py-0.5 rounded text-xs" title="{{ $od->override_reason }}">Override{{ $od->override_reason ? ': '.$od->override_reason : '' }}</span> @endif
                        @if($od->protocolDrug->notes) <span class="text-gray-500">{{ $od->protocolDrug->notes }}</span> @endif
                    </td>
                </tr>
                @endforeach
            </tbody>
    ...
    <div class="mt-4 text-center text-xs text-gray-400">
        Printed: {{ now()->format('d M Y H:i') }} &mdash; {{ env('HOSPITAL_NAME') }} &mdash; {{ $order->order_number }} &mdash; CONFIDENTIAL MEDICAL DOCUMENT
    </div>
</body>
</html>
